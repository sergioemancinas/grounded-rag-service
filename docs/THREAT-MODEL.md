...
| **R**epudiation | Tool invocation cannot be attributed | Mitigated for MCP | Every MCP tool call emits a structured audit event with caller subject, tool, argument summary, outcome, and duration (`app/audit.py`). Core HTTP routes carry a `request_id` but are not yet audited to the same standard. |
...
| — | No tool audit trail (MCP08) | Structured per-call audit events | **Closed** |
