...
def test_event_emitted_per_call(caplog: pytest.LogCaptureFixture) -> None:
    ...
    with caplog.at_level(logging.INFO, logger="grounded_rag.audit"):
        search("orders", k=2)

    records = [record for record in caplog.records if record.name == "grounded_rag.audit"]
...
def test_subject_recorded(caplog: pytest.LogCaptureFixture) -> None:
    ...
    event = json.loads(next(r.message for r in caplog.records if r.name == "grounded_rag.audit"))
...
def test_failures_recorded_with_error_class(caplog: pytest.LogCaptureFixture) -> None:
    ...
    with caplog.at_level(logging.INFO, logger="grounded_rag.audit"), pytest.raises(ValueError):
        fetch("missing")
